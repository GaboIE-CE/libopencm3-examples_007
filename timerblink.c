...
#define LGREENF GPIO13 // Defines LGREENF as GPIO13 
  ...
  /* Enable TIM1 clock. */
	 ...
	 */
		timer_set_mode(TIM1, TIM_CR1_CKD_CK_INT,
		       TIM_CR1_CMS_EDGE, TIM_CR1_DIR_UP); // Cambiando el modo del timer a TIM_CR1_CMS_EDGE
    ...
    timer_set_period(TIM1, 13125); // Setting ARR register = 1000ms valor para periodo de 20 ms
    //timer_set_period(TIM1, 256.3); // 62 ms
    // timer_set_period(TIM1,2563); // Valor original
    // Calcular este periodo de conteo, arriba
	/* Set the initual output compare value for OC1. */
    //	timer_set_oc_value(TIM1, TIM_OC1, 656.25); // no usar los negativos, valor para duty dycle del 5%
    timer_set_oc_value(TIM1, TIM_OC1,1321);  // Valor para el 10%
...
int main(void)
{
	...
	while(1){
timer_set_oc_value(TIM1, TIM_OC1,1321);
